...
export function generateOpenCDAConfig(
  ...
  if (oc.enable_background_traffic) {
    ...
    lines.push('  range:');
    lines.push(
      `    - [${bgR.x_min}, ${bgR.x_max}, ${bgR.y_min}, ${bgR.y_max}, ${bgR.z_min}, ${bgR.z_max}, ${oc.bg_vehicle_num}]`,
    );
    lines.push('');
  }
  ...
  if (rsus.length > 0) {
    ...
    rsus.forEach((rsu, i) => {
      ...
      lines.push(`      id: ${i + 1}`);
      lines.push(
        `      # tx_power: ${rsu.tx_power} | frequency: ${rsu.frequency} | range: ${rsu.range} | protocol: ${rsu.protocol}`,
      );
    });
  } else {
    lines.push('  rsu_list:');
  ...
  if (cars.length > 0) {
    ...
    cars.forEach((car, i) => {
      ...
      lines.push(
        `      # model: ${car.model || 'vehicle.tesla.model3'} | speed: ${car.speed ?? oc.max_speed} km/h`,
      );
    });
  } else {
    lines.push('  single_cav_list:');
