...
import type { SharedMouseContext, UseMouseEventsOptions } from './types/IMouseEventsTypes';
import { useEditorRefs } from '../../context/EditorRefsContext';

export function useDblClickHandler(opts: UseMouseEventsOptions,  ctx: SharedMouseContext) {
  const { buildingModelRef, updateSceneGraph } = opts;
  const {sceneRef, cameraRef, loadPointsRef, transformControlsRef, carMeshesRef, 
    pointsArrRef, roadMeshRef, cubeCirclesRef, modeRef} = useEditorRefs()
  return useCallback(
    (e: MouseEvent) => {
      ...
      const intersects  = ctx.raycaster.intersectObjects([...carMeshes, ...cubeCircles.flat(), ...pointsArr]);
      ...
      if (mode.isAddPointModeActive) {
        ...
        if (intersects.length > 0 && intersects[0].object === road) {
          ...
          if (ctx.raycaster.ray.intersectPlane(plane, pt)) addAt(pt.x, pt.y, pt.z);
        }
        return;
      }
      ...
      if (selectedIdx >= 0 && mode.isAddedPoints) {
        ...
        if (roadHits.length > 0 && roadHits[0].object === road) {
          ...
          const pt = roadHits[0].point;
          if (currentSelectedId) useEditorStore.getState().addPoint(currentSelectedId, pt.x, pt.y, pt.z);
