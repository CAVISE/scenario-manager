...
def _compute_yaw(sx: float, sy: float, sz: float,
    ...
    if carla_map is not None:
        try:
            ...
            if wp is not None:
                # Normalize to [-180, 180] — CARLA occasionally returns values
                # outside this range on certain road geometries.
                raw_yaw = wp.transform.rotation.yaw
                yaw = (raw_yaw + 180.0) % 360.0 - 180.0
                log.info("yaw=%.1f deg (road waypoint, raw=%.1f)", yaw, raw_yaw)
                return yaw
