...
export function useClickHandler(ctx: SharedMouseContext) {
  ...
  const {sceneRef, transformControlsRef, cameraRef, roadMeshRef, carMeshesRef, 
    pointsArrRef, cubeCirclesRef, modeRef, currentCarRef, currentColorRef, loadPointsRef, 
    rsuMeshesRef, pedestrianMeshesRef} = useEditorRefs()
  ...
  return useCallback(
    (e: MouseEvent) => {
      ...
      if (!currentCarRef.current) currentCarRef.current = 'car_' + Math.floor(Math.random() * 1000);

      if (mode.isAddCarModeActive) {
        const hits = ctx.raycaster.intersectObjects([...carMeshes, ...pointsArr, road], true);
        if (hits.length > 0 && hits[0].object === road) {
          const pt = hits[0].point;
          useEditorStore.getState().addCar(pt.x, pt.y, pt.z, currentCarRef.current, currentColorRef.current, 60);
      ...
      if (lidarHit.length > 0) {
        ...
        useEditorStore.getState().selectObject({ type: 'lidar', id: root.userData.id });
      ...
      if (carHit.length > 0) {
        ...
        useEditorStore.getState().selectObject({ type: 'car', id: root.userData.id ?? (carHit[0].object as THREE.Mesh).userData.id });
        transformControls.attach(root);
        onSelectObject(null);
      ...
      if (rsuHit.length > 0) {
        ...
        if (rsuId) useEditorStore.getState().selectObject({ type: 'rsu', id: rsuId });
        transformControls.attach(root);
        ...
        if (best) {
          ...
          for (let i = 0; i < cubeCircles.length; i++) {
            ...
            if (j !== -1) {
              ...
              const pt = useEditorStore.getState().points.filter(p => p.carId === carId)[j];
              pointId = pt?.id;
              break;
            }
          }
          useEditorStore.getState().selectObject(pointId ? { type: 'point', id: pointId } : null);

          transformControls.attach(best);
          onSelectObject({ type: 'point', id: pointId, position: (best as THREE.Mesh).position });
        }
        return;
      }
      ...
      if (pedHit.length > 0) {
        ...
        while (root.parent && root.parent.userData.type === 'pedestrian') root = root.parent;
        if (root.userData.type === 'pedestrian') {
        useEditorStore.getState().selectObject({ type: 'pedestrian', id: root.userData.id });
          transformControls.attach(root);
          onSelectObject({ type: 'pedestrian', id: root.userData.id, position: root.position });
        }
        return;
      }
      ...
      if (bldHit.length > 0) {
        ...
        if (root.userData.type === 'building') {
          ...
          useEditorStore.getState().selectObject(bid ? { type: 'building', id: bid } : null);
          transformControls.attach(root);
          onSelectObject(null);
