...
import { useScenariosListQuery } from '../../hooks/useScenarioQueries';
import { handleLoad } from '../../../components/Statuses/RightPanel/components/ScenarioControlWidget/Handlers/scenario.handlers';
...
const UploadScenariosModal: React.FC<UploadScenariosModalProps> = ({
  ...
  const { data: scenarios = [], isLoading, isError, error, refetch } = useScenariosListQuery(open);
  ...
  const thumb = selectedScenario ? previewSrc(selectedScenario.preview) : undefined;
          ...
          <>
            {isError ? (
              <Alert severity="error" sx={{ mb: 2 }} action={
                ...
                {error instanceof Error ? error.message : 'Failed to load scenario list'}
              </Alert>
            ) : null}
            {isLoading ? (
              ...
              <Grid container spacing={2}>
                {scenarios.length === 0 && !isLoading ? (
                  <Grid item xs={12}>
                    <Typography color="text.secondary">No saved scenarios</Typography>
                  </Grid>
                ) : null}
                {scenarios.map((scenario) => (
                  <Grid item xs={12} sm={6} md={4} key={scenario.scenario_id}>
                    <ScenarioCard onClick={() => handleSelectScenario(scenario)}>
                      <AspectRatio ratio="16/9" sx={{ minHeight: 140, bgcolor: '#eee' }}>
