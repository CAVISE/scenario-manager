  ...
  return {
    load_file() {
      ...
      input.addEventListener('change', (ev: Event) => {
        ...
        reader.onload = e => {
          if (typeof e.target?.result === 'string') loadFile(e.target.result, true);
        };
    ...
    fitView() {
      ...
      if (odrMeshes.refline_lines) fitViewToObj(odrMeshes.refline_lines, camera, {} as never);
    },
    ...
    addCube() {
      ...
      carMeshesRef.current.forEach(m => useEditorStore.getState().updateCar(m.userData.id, { rotation: m.rotation.z }));
      loadPoints();
    },
    ...
    deleteCube() {
      ...
      if (selectedCarId) useEditorStore.getState().removePointsByCarId(selectedCarId);
