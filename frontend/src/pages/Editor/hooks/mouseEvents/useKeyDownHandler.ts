...
import { UseMouseEventsOptions } from './types/IMouseEventsTypes';
import { useEditorRefs } from '../../context/EditorRefsContext';

export function useKeyDownHandler(opts: UseMouseEventsOptions) {
  const { onDeleteCube, updateSceneGraph} = opts;
  const {sceneRef, carMeshesRef, transformControlsRef, pointsArrRef, 
    rsuMeshesRef, pointsObjsRef, cubeCirclesRef, modeRef, loadPointsRef} = useEditorRefs()
    ...
    (e: KeyboardEvent) => {
      ...
      if (e.key === 'Escape') {
        ...
        cubeCircles.forEach((arr, ai) => {
          ...
          arr.forEach((c, pi) => {
            if (pts[pi]) useEditorStore.getState().updatePoint(pts[pi].id, { x: c.position.x, y: c.position.y, z: c.position.z });
          });
        });
      ...
      if (type === 'point') {
        ...
        if (idx !== -1) {
          ...
          const rsuId = root.userData.id as string | undefined;
            if (rsuId) {
              const storeIdx = useEditorStore.getState().RSUs.findIndex(r => r.id === rsuId);
