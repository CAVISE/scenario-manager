...
import { Building, Car, Lidar, Pedestrian, Point, RSU } from "../../../../../../../store/types/useEditorStoreTypes";
import { useEditorStore } from "../../../../../../../store/useEditorStore";
import * as THREE from 'three';
export function buildScenarioPayload(): ScenarioPayload {
  const s = useEditorStore.getState();
  const canvas = document.querySelector('#ThreeJS canvas') as HTMLCanvasElement | null;
  return {
    ...
    scenario: [
      {
        ...
        path: s.cars.map((car: Car) => ({
          ...
          points: s.points.filter((p: Point) => p.carId === car.id).map((p: Point, i: number) => ({ id: i, x: p.x, y: p.y, z: p.z })),
          lidars: s.lidars.filter((l: Lidar) => l.carId === car.id).map((l: Lidar) => ({
            x: l.x, y: l.y, z: l.z,
      ...
      },
      {
        ...
        path: s.buildings.map((b: Building) => ({
          ...
          height: b.height, material: b.material,
        })),
      },
      {
        vehicle: 'pedestrian',
