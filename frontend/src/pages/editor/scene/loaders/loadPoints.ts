...
import { segments, radius } from './loadPoints.types';
import type { LoadPointsContext } from './loadPoints.types';
import type { ConnectLinesContext } from './loadPoints.types';
  ...
  cubeCircles.forEach((circleArray, index) => {
    if (circleArray) {
      circleArray.forEach((circle) => {
        ...
        (Array.isArray(circle.material)
          ? circle.material
          : [circle.material as THREE.Material]
        ).forEach((m) => m?.dispose());
      });
      cubeCircles[index] = [];
    }
