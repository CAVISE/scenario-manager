...
import { UsePedestrianMeshSyncOptions } from './types/usePedestrianMeshSyncTypes';
...
export function usePedestrianMeshSync({updateSceneGraph}: UsePedestrianMeshSyncOptions) {
  const [modelReady, setModelReady] = useState(!!pedestrianModel);
  const pedestrians = useEditorStore(s=>s.pedestrians)
  const {sceneRef, pedestrianMeshesRef, pedestrianObjsRef, transformControlsRef} = useEditorRefs()
  useEffect(() => {
    if (!pedestrianModel) {
  ...
  const trySync = (attempts = 0) => {
    ...
    pedestrians.forEach(ped => {
      const exists = pedestrianMeshesRef.current.find(p => p.userData.id === ped.id);
      if (exists) return;
