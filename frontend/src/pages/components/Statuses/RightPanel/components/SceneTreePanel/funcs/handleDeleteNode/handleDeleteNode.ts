...
export function handleDeleteNode({id, name, e, carMeshesRef, cubeCirclesRef, pointsArrRef, sceneRef, transformControlsRef, onDetach}:handleDeleteNodeProps){
  e.stopPropagation();
        ...
        if (meta.label === 'CAR') {
          ...
          if (rsuIdx !== -1) {
            const meshIdx = pointsArrRef.current.findIndex(m => m.userData.id === id);
            if (meshIdx !== -1) {
