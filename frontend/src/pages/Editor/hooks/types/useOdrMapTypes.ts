import type { OpenDriveMapInstance, OdrMapConfig, OdrRoadNetworkMesh } from '../../types/editorTypes';
import type { PickingScenes, PickingMaterials } from '../types/useThreeSetupTypes';
import * as THREE from 'three';
import { MapControls, TransformControls } from 'three-stdlib';

export interface OpenDriveModule {
  FS_unlink(path: string): void;
  FS_createDataFile(parent: string, name: string, data: string, canRead: boolean, canWrite: boolean): void;
  get_refline_segments(map: OpenDriveMapInstance, resolution: number): {
    vertices: { size(): number; get(i: number): number; delete(): void };
    indices:  { size(): number; get(i: number): number; delete(): void };
  };
  get_road_network_mesh(map: OpenDriveMapInstance, resolution: number): OdrRoadNetworkMesh;
  HEAP8: { length: number };
  OpenDriveMap: new (path: string, config: OdrMapConfig) => OpenDriveMapInstance;
}
export const COLORS = {
  road: 1.0, roadmark: 1.0,
  lane_outline: 0xffffff, roadmark_outline: 0xffffff,
  ref_line: 0x69f0ae,
  lane_highlight: 0xffff00, roadmark_highlight: 0xffff00,
};
...
export interface OdrMapMeshes {
  ...
  road_network_mesh:     THREE.Mesh<THREE.BufferGeometry, THREE.MeshPhongMaterial> | null;
  roadmarks_mesh:        THREE.Mesh<THREE.BufferGeometry, THREE.MeshBasicMaterial> | null;
