...
export function createTransformListener(opts: CreateTransformListenerOptions): () => void {
  const {
    transformControls,
    ...
    try {
      ...
      if (type === 'car' && id) {
        ...
        useEditorStore.getState().updateCar(id, {
          ...
          rotation: obj.rotation.z, scale: obj.scale.x,
        });
      }

      if (type === 'point' && id) {
        useEditorStore.getState().updateRSU(id, { x: obj.position.x, y: obj.position.y, z: obj.position.z });
      }
      ...
      if (type === 'building' && id) {
        useEditorStore.getState().updateBuilding(id, {
          x: obj.position.x, y: obj.position.y, z: obj.position.z,
          rotation: obj.rotation.y, scale: obj.scale.x,
          material: obj.userData.material, height: obj.userData.height,
        });
      }
      ...
      if (type === 'circle') {
        ...
        for (let i = 0; i < circles.length; i++) {
          ...
          if (ci !== -1) {
            ...
            if (pt) useEditorStore.getState().updatePoint(pt.id, { x: obj.position.x, y: obj.position.y, z: obj.position.z });
            useEditorStore.getState().selectObject({ type: 'point', id: pt?.id || '' });
            break;
          }
        }
      }
  ...
  return () => transformControls.removeEventListener('objectChange' as never, handler);
}