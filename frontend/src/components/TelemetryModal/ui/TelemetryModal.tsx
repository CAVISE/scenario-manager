...
import {
  ...
  telemetryModalAspectRatioStyles,
  ...
  const groupImagesByTab = (
    ...
    Object.entries(modules).forEach(([path, module]) => {
      ...
      if (/(route|planned|actual|fig\d+)/i.test(lowerFileName)) {
        ...
        /(event|accelerometer|gyro|velocity|collide|offroad|stuck|traffic)/i.test(
          lowerFileName,
        )
      ) {
        result.telemetry.push({ url: imageUrl, name: displayName });
      } else if (
        /(localization|kinematics)/i.test(lowerFileName) ||
        /^\d+_/.test(fileName)
      ) {
        ...
        {images.map((image, index) => (
          <Grid item xs={12} sm={6} md={4} lg={3} key={`${tabKey}-${index}`}>
            <ImageCard onClick={() => handleImageClick(image.url)}>
              <AspectRatio ratio="4/3" sx={telemetryModalAspectRatioStyles}>
                <img
                  src={image.url}
                  alt={image.name}
                  loading="lazy"
    ...
    <>
      <Modal open={open} onClose={onClose}>
        <ModalContainer>
          ...
          <TabPanel
            sx={{ display: activeTab === 'localization' ? 'block' : 'none' }}
          >
