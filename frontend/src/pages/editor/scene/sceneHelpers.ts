...
import type { StdMap, StdVec, OdrMeshUnion } from './sceneHelpers.types';

export function get_geometry(
  odr_meshunion: OdrMeshUnion,
...
export function applyVertexColors(
  ...
  for (
    let i = 0;
    i < count * buffer_attribute.itemSize;
    i += buffer_attribute.itemSize
  ) {
    colors[i] = color.r;
    colors[i + 1] = color.g;
    colors[i + 2] = color.b;
  }
  (buffer_attribute as THREE.BufferAttribute).array.set(
    colors,
    offset * buffer_attribute.itemSize,
  );
}

export function getStdMapKeys<K>(
  std_map: StdMap<K, number>,
  delete_map = false,
): K[] {
  const map_keys: K[] = [];
...
export function getStdMapEntries<K, V>(std_map: StdMap<K, V>): [K, V][] {
  ...
  for (const key of keys) map_entries.push([key, std_map.get(key)]);
  return map_entries;
}
