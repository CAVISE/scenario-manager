...
export function useDatGui(options: UseDatGuiOptions) {
  ...
  useEffect(() => {
    ...
    gui
      .add(P, 'resolution', { Low: 1.0, Medium: 0.3, High: 0.02 })
      .name('📏 Detail')
      .onChange(() => onReloadMap());
    gui.add(P, 'spotlight').name('🔦 Spotlight');
    gui.add(ACTIONS, 'fitView').name('⟲ Reset Camera');

    const viewF = gui.addFolder('View');
    viewF
      .add(P, 'view_mode', { Default: 'Default', Outlines: 'Outlines' })
      .name('View Mode')
      .onChange((val: string) => {
        const m = meshVisRef.current;
        if (m.roadmarks_mesh)
          m.roadmarks_mesh.visible = val === 'Default' && P.roadmarks;
      });
    viewF
      .add(P, 'ref_line')
      .name('Reference Line')
      .onChange((val: boolean) => {
        if (meshVisRef.current.refline_lines)
          meshVisRef.current.refline_lines.visible = val;
      });
    viewF
      .add(P, 'roadmarks')
      .name('Roadmarks')
      .onChange((val: boolean) => {
        const m = meshVisRef.current;
        if (m.roadmarks_mesh)
          m.roadmarks_mesh.visible = val && P.view_mode !== 'Outlines';
        if (m.roadmark_outline_lines)
          m.roadmark_outline_lines.visible = val && P.view_mode !== 'Outlines';
      });
    viewF
      .add(P, 'wireframe')
      .name('Wireframe')
      .onChange((val: boolean) => {
        if (meshVisRef.current.road_network_material)
          meshVisRef.current.road_network_material.wireframe = val;
      });
    ...
    ctrlF
      .add(ACTIONS, 'model')
      .name('Car name')
      .onChange((val: string) => onNameChange(val));

    const wrapper = document.createElement('div');
    wrapper.style.cssText =
      'margin:8px 10px;display:flex;align-items:center;justify-content:space-between;';
    ...
    colorBox.style.cssText =
      'width:28px;height:18px;background:#00ff00;cursor:pointer;border:1px solid #555;border-radius:2px;';

    const pickerContainer = document.createElement('div');
    pickerContainer.style.cssText =
      'position:absolute;right:0;top:22px;z-index:9999;display:none;';
    ...
    saveF
      .add(scenarioSettingsRef.current, 'scenario_name')
      .name('name scanario');

    saveF
      .add(scenarioSettingsRef.current, 'weather', WEATHER_OPTIONS)
      .name('Weather');
