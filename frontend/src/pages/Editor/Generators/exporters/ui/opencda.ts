...
function pushYamlNode(lines: string[], indent: string, value: unknown): void {
  ...
  if (value != null && typeof value === 'object') {
    pushYamlObject(lines, indent, value as Record<string, unknown>);
    return;
  }
...
function pushCameraBlock(
  ...
  const num = effectiveCamNum(requestedNum);
  ...
  if (num > 0) {
    ...
    for (let i = 0; i < num; i++) {
      lines.push(`${indent}    - ${CAM_POSITIONS[i]}`);
    }
  }
}
...
function pushLocalizationBlock(
  ...
  if (includeMetrics) {
    pushLocalizationMetrics(lines, oc.metrics.localization_trace_warmup, `${indent}  `);
  }
...
function pushRsuBehaviorServices(
  ...
  for (const svc of services) {
    if (svc.type === 'aim_server') {
      ...
      if (svc.debug != null)
        lines.push(`${indent}    debug: ${svc.debug}`);
      ...
      if (svc.model) lines.push(`${indent}    model: "${svc.model}"`);
      if (svc.underling_model)
        lines.push(`${indent}    underling_model: "${svc.underling_model}"`);
      if (svc.hidden_channels != null)
        lines.push(`${indent}    hidden_channels: ${svc.hidden_channels}`);
      if (svc.weight) lines.push(`${indent}    weight: "${svc.weight}"`);
...
function pushExtendedVehicleBase(lines: string[], oc: SimulationConfig['opencda']): void {
...
function pushVehicleBaseBehavior(
  ...
  lines.push('  behavior:');
  if (oc.export_profile === 'aim_check') {
  ...
  lines.push(`      trajectory_update_freq: ${oc.local_planner.trajectory_update_freq}`);
  lines.push(`      waypoint_update_freq: ${oc.local_planner.waypoint_update_freq}`);
...
function pushVehicleLocalization(
  ...
  if (oc.export_profile === 'aim_check') {
    ...
    lines.push(
      `        show_animation: ${oc.vehicle_localization_debug_animation}`,
    );
    return;
  }
...
export function generateOpenCDAConfig(
  ...
  if (aimStyle) {
    lines.push(
      `  Scenario testing aim models — generated by ScenarioManager`,
    );
  } else {
    ...
    lines.push(`  Merge with assets/opencda/base.yaml if fields are missing.`);
  }
  lines.push('');

  if (!aimStyle && oc.export_attacks) {
  ...
  if (oc.export_world_client_host && !aimStyle) {
    lines.push(`  client_host: ${oc.world_client_host}`);
  }
  if (!aimStyle) {
    lines.push('  weather:');
  ...
  pushCameraBlock(lines, '      ', oc.vehicle_camera_visualize, oc.vehicle_cam_num);
  lines.push('      lidar:');
  lines.push(`        visualize: ${oc.lidar_visualize}`);
  ...
  if (!aimStyle && oc.export_full_vehicle_base) {
  ...
  if (!aimStyle && oc.enable_background_traffic) {
  ...
  if (!aimStyle && oc.export_platoon_base) {
    pushPlatoonBase(lines, oc.platoon_base, oc.export_metrics);
  }

  if (!aimStyle && oc.export_coop_perception) {
  ...
  if (rsus.length > 0) {
    lines.push('  rsu_list:');
    rsus.forEach((rsu, i) => {
      lines.push(`    - name: ${rsuDisplayName(rsu, i)}`);
      lines.push(
  ...
  if (cars.length > 0) {
    ...
    cars.forEach((car, i) => {
      const yawDeg = car.rotation
        ...
        : '0.0';
      const carPoints = car.id
        ? points.filter((p) => p.carId === car.id)
        : [];
      const dest =
        carPoints.length > 0
      ...
      lines.push(`    - name: ${cavDisplayName(car, i)}`);
      ...
      if (car.opencda_carla_model?.trim()) {
        lines.push(`      model: "${car.opencda_carla_model.trim()}"`);
      }
